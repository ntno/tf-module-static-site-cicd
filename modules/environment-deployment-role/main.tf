...
locals {
  ...
  cloudfront_id_is_blank   = can(length(var.cloudfront_distribution_id)) && length(var.cloudfront_distribution_id) != 0 ? false : true
  enable_cloudfront_policy = local.cloudfront_id_is_blank ? false : true
